...
测试账号登录后，所有数据**只存在你自己的浏览器**里（localStorage 单独隔离一份，不影响其他用户），不会写入 Firestore，也不会被其他用户看到。左下角徽标会显示 **「本地演示」**。

- 首次登录会自动填入一份**当前月份的示例账目**（午饭、咖啡、聚餐等十来笔，含一组现金流参数），方便直接看到完整界面
- 可以随意添加、修改、删除，刷新会保留这些改动
- 想"恢复出厂"重新看演示数据：在「设置」里点 **重置数据**（清空浏览器缓存也行）
- 换设备 / 清浏览器缓存 → 数据丢失，再次登录会重新看到初始示例
