...
| 账号 | 密码 |
|------|------|
| `test@bill-app.demo` | `test` |

测试账号登录后，所有数据**只存在你自己的浏览器**里（localStorage），不会写入 Firestore，也不会被其他用户看到。左下角徽标会显示 **「本地演示 · 不占额度」**。刷新页面数据会保留，但换设备或清浏览器缓存会丢失。
...
再添加一个：邮箱 `test@bill-app.demo`，密码 `test`。这个账号会被前端识别为「演示模式」，所有写操作只走本地 localStorage，不消耗 Firestore 额度。
